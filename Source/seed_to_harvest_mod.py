...
##mod
#case id replaced by site id
#crop name encoded into the event name
...
def eventtime_MACD(ndvits, seasondf, sitecdl, siteid, siteloc, crop_usuals, season_usuals, start_year=None, end_year=None, smooth='ALL'):
    ...
    for y in tqdm(np.arange(start_year, end_year+1, 1), desc='Site '+str(siteid), unit='years', leave=False):
        ...
        # Write to list
        if len(s1dL) != 0:
            crop=crop_id[int(sitecdl.loc[y][0])]
            ...
            lst2=itwritein(lst2, ['First Snow Previous','Last Snow',crop+' Emergence', crop+' Maturity', crop+' Senescence', crop+' Dormancy', 'First Snow'])
